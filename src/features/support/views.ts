...
export function supportPanel(state: SupportState = { status: 'idle', receipts: [] }): string {
  ...
  return `<div class="panel support-panel">
    ...
    <p class="section-help support-note">Plain Lightning and on-chain routes are not shown here because they bypass the
    public receipt trail. The meter below counts only wallet-provider-signed zap receipts — no analytics, no account.</p>
    ${fundingPanel(state)}
  </div>`;
}
