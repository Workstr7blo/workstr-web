...
function programMuscleMap(program: RelayProgram, exercises: Exercise[]): string {
  ...
  return RECOVERY_BODY_SVG.replace(/<polygon([^>]*data-muscle="([^"]+)"[^>]*)>/g, (_match, attrs: string, muscle: string) => {
    if (primary.has(muscle)) return `<polygon${attrs} style="fill:var(--sovereign-purple);opacity:.95">`;
    if (secondary.has(muscle)) return `<polygon${attrs} style="fill:var(--purple-2);opacity:.5">`;
    return `<polygon${attrs} style="fill:#2a1d40">`;
  }).replace(/<polygon((?:(?!data-muscle)[^>])*)>/g, '<polygon$1 style="fill:#1a1228">');
}
