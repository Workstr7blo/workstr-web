...
switch. Lightning zaps and the NWC wallet were removed in #218. Run this when cutting a
release.

- [ ] Settings → Support Workstr shows the QR code, the shortened address and Copy address,
      with Monero tips both off and on.
- [ ] The QR scans in a Monero wallet and resolves to the address in `src/core/funding.ts`,
      which is also the address in the donate section on workstr.fit.
- [ ] Copy address copies the whole address; Open wallet hands a `monero:` link to an
      installed wallet.
- [ ] Monero tips off: nothing in Settings is orange and the account pill has no medallion.
      On: the address section, the orange card edge and the medallion appear, and any card
      left open elsewhere on Settings stays open.
- [ ] No zap button, Lightning address, sats figure or funding meter appears anywhere.
- [ ] **Upgrade path.** On a device that had a zap wallet connected on the previous
      release, open the new build once: Application → IndexedDB no longer lists
      `workstr-secure-nwc-v1`.
