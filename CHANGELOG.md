...
- Your exercise library now syncs between devices. Encrypted sync carried programs, workouts,
  body weight and settings but never the library, so an exercise imported, starred or deleted
...
- Exercise photos are fetched at the size they are drawn instead of at full resolution.
  ...
  high-density screen. What Workstr stores is untouched - the original image address still
  goes into your database, your published programs and your backups, so the pictures keep
  working if the resizing service ever goes away.
...
- **Settings is now a compact control center.** Account, Beast Mode, Data & Sync, Zap wallet,
  Training Preferences, Support Workstr, and Advanced are grouped as disclosure categories;
  encrypted relay behavior is described as sync/auto-sync, JSON export/import is presented as
  manual backup, support is compact by default, equipment is shown as a concise inline section,
  and diagnostics are tucked into Advanced.
