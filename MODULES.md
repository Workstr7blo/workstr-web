...
| Workout History | `src/features/train/views.ts` | `src/features/train/history-calendar.ts`, `src/features/train/history-model.ts`, `src/core/dates.ts`, `src/app/preferences-controller.ts`, `src/app/session-persistence.ts`, `src/app/state.ts` | `tests/history-calendar.test.ts`, `tests/history-model.test.ts`, `tests/dates.test.ts` |
